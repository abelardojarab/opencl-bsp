...
#check if kernel_comp_a10/vector_add exists; rename if it does
if [ -d kernel_comp_a10 ]; then
    if [ -d kernel_comp_a10/vector_add ]; then
        mv kernel_comp_a10/vector_add kernel_comp_a10/vector_add_$(date +%Y%m%d%H%M%S)
    fi
else
