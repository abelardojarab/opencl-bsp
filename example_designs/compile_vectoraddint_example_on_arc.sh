...
#check if kernel_comp_a10/vector_add_int exists; rename if it does
if [ -d kernel_comp_a10 ]; then
    if [ -d kernel_comp_a10/vector_add_int ]; then
        mv kernel_comp_a10/vector_add_int kernel_comp_a10/vector_add_int_$(date +%Y%m%d%H%M%S)
    fi
else
