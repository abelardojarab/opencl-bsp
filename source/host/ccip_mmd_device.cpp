...
int CcipDevice::next_mmd_handle {
1};
...
void CcipDevice::initialize_local_cpus_sysfs()
{
	...
	// but API does not currently expose the device temperature.
	int dev_num = 0xFFFFF & fpga_obj_id;
	snprintf(localnumapath, MAX_LEN,
		 "/sys/class/fpga/intel-fpga-dev.%d/device/numa_node", dev_num);
	snprintf(localcpupath, MAX_LEN,
		 "/sys/class/fpga/intel-fpga-dev.%d/device/local_cpus",
		 dev_num);
	...
	if (tmp) {
		...
		for (ndx = nread; ndx >= 0; ndx--) {
			...
			default:
				...
				unsigned long val =
				    strtol(&cpustr[ndx], NULL, 16);
				if (val & 0x1)
					CPU_SET(cpunum, &numa.afu_cpuset);
				cpunum++;
				if (val & 0x2)
					CPU_SET(cpunum, &numa.afu_cpuset);
				cpunum++;
...
int CcipDevice::program_bitstream(uint8_t * data, size_t data_size)
{
	...
	if (kernel_interrupt_thread) {
		...
		if (!kernel_interrupt_thread->initialized()) {
			fprintf(stderr,
				"Error initializing kernel interrupts\n");
			return false;
		}
	}
...
float CcipDevice::get_temperature()
{
	...
	if (fme_sysfs_temp_initialized) {
		std::ifstream sysfs_temp(fme_sysfs_temp_path,
					 std::ifstream::in);
		sysfs_temp >> temp;
		sysfs_temp.close();
	}
...
void CcipDevice::set_kernel_interrupt(aocl_mmd_interrupt_handler_fn fn,
				      void *user_data)
{
	if (kernel_interrupt_thread) {
...
void CcipDevice::set_status_handler(aocl_mmd_status_handler_fn fn,
				    void *user_data)
{
	event_update = fn;
...
int CcipDevice::read_block(aocl_mmd_op_t op, int mmd_interface, void *host_addr,
...
int CcipDevice::write_block(aocl_mmd_op_t op, int mmd_interface,
			   ...
			   size_t src_offset, size_t dst_offset, size_t size)
{
	...
	if (mmd_interface == AOCL_MMD_MEMORY) {
		...
		while (bytes_left != 0) {
			size_t chunk =
			    bytes_left >
			    MMD_COPY_BUFFER_SIZE ? MMD_COPY_BUFFER_SIZE :
			    bytes_left;
			...
			status =
			    read_block(NULL, AOCL_MMD_MEMORY, mmd_copy_buffer,
				       read_offset, chunk);
			if (status != 0)
				break;
			status =
			    write_block(NULL, AOCL_MMD_MEMORY, mmd_copy_buffer,
					write_offset, chunk);
			if (status != 0)
				break;
			read_offset += chunk;
...
fpga_result CcipDevice::write_mmio(const void *host_addr, size_t mmio_addr, size_t size)
{
	...
	const uint64_t *host_addr64 =
	    static_cast < const uint64_t * >(host_addr);
	while (size >= 8) {
