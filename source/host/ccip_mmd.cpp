...
static std::string get_offline_board_names()
{
	...
	toks =
	    static_cast <
	    fpga_token * >(malloc(num_matches * sizeof(fpga_token)));
	if (toks == NULL) {
	...
	for (unsigned int i = 0; i < num_matches; i++) {
		...
		if (res == FPGA_OK) {
			...
			boards.
			    append(CcipDevice::get_board_name(prefix, obj_id));
			if (i < num_matches - 1)
				boards.append(";");
		} else {
...
int AOCL_MMD_CALL aocl_mmd_reprogram(int handle, void *data, size_t data_size)
{
	...
	struct acl_pkg_file *pkg =
	    acl_pkg_open_file_from_memory((char *)data, data_size,
					  ACL_PKG_SHOW_ERROR);
	struct acl_pkg_file *fpga_bin_pkg = NULL;
	struct acl_pkg_file *search_pkg = pkg;
	ACL_DCP_ERROR_IF(pkg == NULL, return AOCL_INVALID_HANDLE,
	...
	if (acl_pkg_section_exists(pkg, ACL_PKG_SECTION_FPGA_BIN, &fpga_bin_len)
	    && acl_pkg_read_section_transient(pkg, ACL_PKG_SECTION_FPGA_BIN,
					      &fpga_bin_contents)) {
		fpga_bin_pkg =
		    acl_pkg_open_file_from_memory((char *)fpga_bin_contents,
						  fpga_bin_len,
						  ACL_PKG_SHOW_ERROR);
	...
	if (acl_pkg_section_exists
	    (search_pkg, ACL_PKG_SECTION_DCP_GBS_GZ, &acl_gbs_gz_len)
	    && acl_pkg_read_section_transient(search_pkg,
					      ACL_PKG_SECTION_DCP_GBS_GZ,
					      &acl_gbs_gz_contents)) {
		void *gbs_data = NULL;
		size_t gbs_data_size = 0;
		int ret =
		    inf(acl_gbs_gz_contents, acl_gbs_gz_len, &gbs_data,
			&gbs_data_size);

		if (ret != Z_OK) {
			fprintf(stderr,
				"aocl_mmd_reprogram error: GBS decompression failed!\n");
			if (gbs_data)
				free(gbs_data);
			return AOCL_INVALID_HANDLE;
		}
		...
		int res =
		    afu->program_bitstream(static_cast < uint8_t * >(gbs_data),
					   gbs_data_size);
		      ...
		      void *param_value, size_t * param_size_ret)
{
	...
	case AOCL_MMD_BOARD_NAME:
		{
			std::ostringstream board_name;
			board_name << "PAC Arria 10 Platform" << " (" << dev->
			    get_dev_name() << ")";
...
int AOCL_MMD_CALL aocl_mmd_set_interrupt_handler(int handle,
...
int AOCL_MMD_CALL aocl_mmd_write(int handle,
				 aocl_mmd_op_t op,
				 size_t len,
				 const void *src,
...
int AOCL_MMD_CALL aocl_mmd_read(int handle,
				aocl_mmd_op_t op,
				size_t len,
...
int AOCL_MMD_CALL aocl_mmd_copy(int handle,
				aocl_mmd_op_t op,
				size_t len,
				...
				size_t dst_offset)
{
	...
	CcipDevice *dev = get_device(handle);
	if (dev)
		return dev->copy_block(op, mmd_interface, src_offset,
				       dst_offset, len);
	else
		return -1;
	//TODO: handle error condition if dev null
